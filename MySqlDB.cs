      ...
      public static readonly string connectionString = "server=localhost;user=root;database=playandbay;port=3306;password=Galka91;";
        ...
        public static string ItemID()
        {
            ...
            try
            {
                ...
                conn.Open();
                //тело запроса!!!!!!!!!!
                string sql = "SELECT ifnull(max(pk_i_id),0)+1 FROM oc_t_item;";
        ...
        public static bool IsNewAd(int id) 
        {
            ...
            try
            {
                ...
                conn.Open();
                //тело запроса!!!!!!!!!!
                string sql = "SELECT count(*) FROM fct_grabber_avito where avito_id=@Id";
                MySqlCommand cmd = new MySqlCommand(sql, conn);
                cmd.Connection = conn;
                cmd.CommandText = sql;
                cmd.Prepare();
      ...
      public static string ResourceID() 
        {
            ...
            try
            {
                ...
                conn.Open();
                //тело запроса!!!!!!!!!!
                string sql = "SELECT ifnull(max(pk_i_id),0)+1 FROM oc_t_item_resource;";
                MySqlCommand cmd = new MySqlCommand(sql, conn);
                object result = cmd.ExecuteScalar();
                if (result != null)
                {
        ...
        public static void InsertFctAvitoGrabber(Dictionary<PartsPage,IEnumerable<string>> list,string index,string url,string section) 
        {
            if (list != null && list.Count>0) 
            {
                ...
                try
                {
                    ...
                    conn.Open();
                    //Тело запроса!!!!!!!
                    string sql = @" insert into fct_grabber_avito (id_resource_list, avito_id,url, title, tel_num, author, price, city, avito_section, user_section, description)
        ...
        public static void InsertItemResource(string resourceid, string url)
        {
            if (resourceid != null)
            {
                ...
                try
                {
                    ...
                    conn.Open();
                    //Тело запроса!!!!!!!
                    string sql = @" insert into oc_t_item_resource
        ...
        public static void InsertassGrabberAvitoResourceList(string index1, string index2)
        {
            if (index1 != null)
            {
                ...
                try
                {
                    ...
                    conn.Open();
                    //Тело запроса!!!!!!!
                    string sql = @" insert into ass_grabber_avito_resource_list
        ...
        public static void DeleteUnTransformated()
        {
            ...
            try
            {
                ...
                conn.Open();
                    //Тело запроса!!!!!!!
                string sql = @" delete from ass_grabber_avito_resource_list 
